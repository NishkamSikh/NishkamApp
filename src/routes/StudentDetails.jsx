    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                //                const response = await fetch('https://apisikligar.azurewebsites.net/api/v1/fetchSingleStudentDetail');
                const response = await fetch(`http://localhost:3000/api/v1/fetchSingleStudentDetails/${JSON.parse(searchParams.get('id'))}`);
            ...
            <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
                ...
                <div class="p-2 bg-green-100">
                    ...
                    {StudentData.map((data, index) => (
                        <div><b>Name:</b> {data.firstname} {data.lastname} <b>Parents:</b> {data.Father_Name}, {data.Mother_Name} <b>DOB:</b> {data.dob} <b>Class:</b> {data.class}
                        </div>
