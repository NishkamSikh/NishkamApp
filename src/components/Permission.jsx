...
const Permissions = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch('https://nishkamapi.onrender.com/api/v1/getAllUsers');
      if (!response.ok) {
  ...
  const fetchPages = async () => {
    try {
      const response = await fetch('https://nishkamapi.onrender.com/api/v1/getAllpages');
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
      const result = await response.json();
  ...
  const handleCheckboxChange = async (userId, pageId, isChecked) => {
    ...
    try {
      const response = await fetch('https://nishkamapi.onrender.com/api/v1/permissions', {
        method: 'POST',
        headers: {
